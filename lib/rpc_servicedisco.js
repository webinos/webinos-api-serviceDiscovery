...
(function () {
	...
	var Discovery = function(rpcHandler, params) {
		...
		this.base({
			api: 'internal://webinos.org/internal/api/ServiceDiscovery',
		...
		var search = function (serviceType, callback, options, filter) {
			...
			for (var api in this.registry.getRegisteredObjectsMap()) {
				if (api === serviceType.api ||
						isPartialMatch(api)) {
					logger.log('INFO: [Discovery] '+"search: found matching service(s) for ServiceType: " + api);
					results = results.concat(this.registry.getRegisteredObjectsMap()[api]);
				}
			}
			...
			this.remoteServicesFoundCallbacks[callbackId] = (function(res, refCnt) {
				return function(remoteServices, cId, ignoreCnt) {
					...
					res = res.concat(remoteServices.filter(isPartialMatch));
					refCnt -= 1;
